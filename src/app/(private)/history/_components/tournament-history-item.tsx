...
export const TournamentHistoryItemComponent = ({ item }: TournamentHistoryItemProps) => {
	const isWinner = item.myResult === "WIN";
	const resultColor = isWinner ? "text-blue-500" : "text-red-500";
	const resultText = item.myResult;

	return (
		<div className={twMerge("flex w-full items-start justify-between rounded-lg bg-neutral-300 p-4", "border border-gray-400 shadow-sm")}>
			...
			<div className={twMerge("font-bold text-xl", SuperPixel.className, resultColor)}>{resultText}</div>
		</div>
	);
};
