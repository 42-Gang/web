...
const HistoryList = ({ gameType, history, filter, currentUserId }: HistoryListProps) => {
	const filteredHistory = history.filter((item) => {
		...
		return filter === "win" ? tournamentItem.myResult === "WIN" : tournamentItem.myResult === "LOSS";
	});
	...
	return (
		<div className="max-h-96 w-full max-w-2xl space-y-3 overflow-y-auto">
			{filteredHistory.map((item) => {
				if (gameType === "duel") {
					const duelItem = item as DuelHistoryItem;
					return <DuelHistoryItemComponent key={`duel-${duelItem.tournamentId}`} item={item as DuelHistoryItem} currentUserId={currentUserId} />;
				}
				const tournamentItem = item as TournamentHistoryItem;
				return <TournamentHistoryItemComponent key={`tournament-${tournamentItem.tournamentId}`} item={item as TournamentHistoryItem} />;
			})}
		</div>
	);
};
