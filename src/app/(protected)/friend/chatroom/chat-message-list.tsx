...
import { forwardRef, type ComponentProps, useEffect, useImperativeHandle, useLayoutEffect, useRef, useState } from 'react';
...
export const ChatMessageList = forwardRef<ChatMessageListRef, Props>(
  ...
  useLayoutEffect(() => {
    ...
    const messagesAddedAtTop = previousFirstMessageId !== null && currentFirstMessageId !== previousFirstMessageId;
    ...
    if (messagesAddedAtTop && !shouldScrollToBottom) {
      ...
      list.scrollTop = list.scrollTop + scrollHeightDiff;
  ...
  return (
    ...
    >
      {isLoadingMore && (
        <li className="text-center text-neutral-600 text-sm py-2">이전 메시지 불러오는 중...</li>
      )}
      {messages.map(msg => {
